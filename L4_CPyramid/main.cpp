...
public:
	CMainWin()
	{
		Create(NULL, L"KG&G Laba 5 Pyramid");
		Index = 0;
		PView.RedimMatrix(3);
	}
	CMenu *menu;
...
afx_msg int CMainWin::OnCreate(LPCREATESTRUCT)
{
	...
	subMenu->AppendMenu(MF_STRING, ID_Dr1, _T("Без невидимых граней"));
	subMenu->AppendMenu(MF_STRING, ID_Dr, _T("C невидимыми гранями"));
	this->menu->AppendMenu(MF_POPUP | MF_STRING, (UINT)subMenu->m_hMenu, _T("Пирамида"));
	SetMenu(menu);
	return 0;
}
